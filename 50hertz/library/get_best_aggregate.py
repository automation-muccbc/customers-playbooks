...
def get_volumes_rest(cluster_mgmt_ip, username, password, key_filepath, cert_filepath):
    url = f"https://{cluster_mgmt_ip}/api/storage/volumes?fields=space.size,svm.name,aggregates&svm.name=!*-mc"
    log(f"GET {url}")
    if(username and password):
        response = requests.get(url, auth=(username, password), verify=False)
    else:
...
# get aggregates
def get_aggregates(cluster_mgmt_ip, username, password, key_filepath, cert_filepath, requested_size_mb, requested_volume_count=1, volume_name="", svm_name=""):
    ...
    if volume_name and svm_name:
        ...
        existing_volume = get_existing_volume_rest(cluster_mgmt_ip, username, password, key_filepath, cert_filepath, volume_name, svm_name)
        if existing_volume:
...
def run_module():
    ...
    module_args = dict(
        ...
        svm_name                  = dict(type='str', required=True),
